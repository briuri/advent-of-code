 ...
 * The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size increases to
 * 3.
 ...
 * your puzzle input. Once this process is complete, what is the result of multiplying the first two numbers in the
 * list?
 * 
 * @author Brian Uri!
 */
public class Day10 extends Puzzle {
	...
	/**
	 * Input: One line with integer inputs
	...
	public static List<Integer> getInput(Part part, int fileIndex) {
		...
		if (part == Part.ONE) {
			...
			for (int i = 0; i < rawData.length(); i++) {
				char character = rawData.charAt(i);
	 ...
	 * element in the list), a skip size (which starts at 0), and a sequence of lengths (your puzzle input). Then, for
	 * each length:
	 * 
	 * Reverse the order of that length of elements in the list, starting with the element at the current position.
	 * Move the current position forward by that length plus the skip size.
	 ...
	 * 
	 * Part 2:
	 * Instead of merely running one round like you did above, run a total of 64 rounds. Once the rounds are complete,
	 * you will be left with the numbers from 0 to 255 in some order, called the sparse hash. Your next task is to
	 * reduce these to a list of only 16 numbers called the dense hash. To do this, use numeric bitwise XOR to combine
	 * each consecutive block of 16 numbers in the sparse hash (there are 16 such blocks in a list of 256 numbers).
	 * 
