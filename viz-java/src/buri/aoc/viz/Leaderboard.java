...
import buri.aoc.viz.common.Company;
import buri.aoc.viz.common.OverallTimes;
import buri.aoc.viz.common.Puzzle;
import buri.aoc.viz.common.PuzzleTime;
import buri.aoc.viz.common.PuzzleTimes;
import org.junit.Test;

import buri.aoc.viz.common.BaseLeaderboard;
...
public class Leaderboard extends BaseLeaderboard {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < reps; i++) {
			...
			Process jsonDowload = Runtime.getRuntime().exec("cmd /c start /min C:\\workspace\\aws-stage\\scripts\\aoc-get-json.bat");
			jsonDowload.waitFor();
			...
			Process htmlUpload = Runtime.getRuntime().exec("cmd /c start /min C:\\workspace\\aws-stage\\scripts\\aoc-put-s3.bat");
			htmlUpload.waitFor();
		}
	}
...
//		page.append("\t</ul>\n");
	...
	private String maskName(String name) {
		...
		int maskPoint = 3;
		if (index != -1 && index + maskPoint < builder.length()) {
			int truncate = index + maskPoint;
	...
	private void insertHeader(String year, boolean isLatestPage) {
		...
		page.append("\t<title>Advent of Code - Rankings (").append(year).append(")").append("</title>\n\n");
		if (!isLatestPage) {
		...
		page.append("\n\t</div>\n\n");
		page.append("\t<h1>Advent of Code - Rankings (").append(year).append(")</h1>\n\n");
	}
